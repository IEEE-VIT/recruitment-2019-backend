    ...
    def get_queryset(self):
        ...
        if self.request.method == 'GET' and candidate_interest is not None:  # for Interviewer
            return Candidate.objects.filter(
                Q(called=False) & Q(is_active=True) & Q(round_1_call=False) & Q(round_2_call=False) & Q(
                    interviewer_switch=False)).filter(
            ...
            return Candidate.objects.filter(
                Q(called=True) & Q(is_active=True) & Q(round_1_call=False) & Q(round_2_call=False) & Q(
                    interviewer_switch=False)).filter(
